...
local function addPlayerItem(self, mlist, ply, pteam)
	...
	function but:Paint(w, h)
		...
		if IsValid(ply) && ply:IsPlayer() then
			...
			local tagName, tagColor = hook.Call( "aTag_GetScoreboardTag", nil, ply )
			
			if( ATAG and tagName and tagName != "" ) then
				
				draw.DrawText( tagName, "ScoreboardPlayer", s + ( w * 0.5 ), 9, color_black, TEXT_ALIGN_CENTER )
				draw.DrawText( tagName, "ScoreboardPlayer", s + ( w * 0.5 ), 8, tagColor or color_white, TEXT_ALIGN_CENTER )
				
				-- draw.DrawText( tagName, "ScoreboardPlayer", w - 80, 9, color_black, TEXT_ALIGN_RIGHT )
				-- draw.DrawText( tagName, "ScoreboardPlayer", w - 80, 8, tagColor or color_white, TEXT_ALIGN_RIGHT )
