...
function PANEL:Init()
	...
	self.m_aTagsLabel = EasyLabel(self, " ", "ZSScoreBoardPlayer", COLOR_WHITE)
...
function PANEL:PerformLayout()
	...
	-- if !( self.m_aTagsLabel:GetText() == "" ) then
		-- self.m_PlayerLabel:SetWide( 80 )
		-- self.m_PlayerLabel:SizeToContentsY()
	-- else
	...
	self.m_aTagsLabel:SizeToContents()
	self.m_aTagsLabel:MoveLeftOf( self.m_ScoreLabel, 12 )
	self.m_aTagsLabel:CenterVertical()
...
function PANEL:Refresh()
	...
	local name, color = hook.Call( "aTag_GetScoreboardTag", nil, pl )
	
	self.m_aTagsLabel:SetText( name or "" )
	self.m_aTagsLabel:SetTextColor( color or Color( 255, 255, 255 ) )
